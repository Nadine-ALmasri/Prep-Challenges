...
const filterLinks = (str) => {
    // write your code here
   // const m = new RegExp("www.([a-zA-Z0-9])+.(com|org|net)")
    let search=str.search("w");
   let n= str.search("com")
   let y= str.search("org")
   let x= str.search("net")
   let url=""

   if (n !==-1){
    let t=n-search
    for (let i=search;i<n ;i++ ){
        url=url+str[i]}
        return url+"com"}
    if (y !==-1){
        let t=y-search
        for (let i=search ;i<y ;i++ ){
           url=url+str[i]
...
const samePattern = (str, arr) => {
    // write your code hereconst samePattern = (str, arr) => {
    // write your code here
