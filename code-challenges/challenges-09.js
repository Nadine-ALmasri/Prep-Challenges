...
const employeesBonus = (arr) => {
   ...
   arr.forEach(inc);
    function inc(item , index,arr) {
        
       
        if ( arr[index]['workHours']>8) {
       
        arr[index].salary=((arr[index].salary).slice(0,4))*1+(100)+"$"; }
            else {
                arr[index].salary=((arr[index].salary).slice(0,4))*1+(50)+"$";
            }
